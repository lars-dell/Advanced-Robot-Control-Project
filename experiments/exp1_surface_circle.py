...
def run_experiment_1(
    ...
    """
    Executes Experiment 1: Surface Circle Trajectory with Surface Force Exertion.
    ...
    """
    logger.info("[Exp 1] Setting up environment with Surface Box...")
    # Surface box: center (0.38, 0.0, 0.20), size (0.40, 0.50, 0.40) -> top surface z = 0.40m
    surface_box_cfg = {
        "pos": (0.38, 0.0, 0.20),
        "size": (0.40, 0.50, 0.40)
    }
    ...
    logger.info("[Exp 1] Initializing Hierarchical QP Impedance Controller...")
    ...
    logger.info(f"[Exp 1] Circle Center: {circle_center}, Radius: {radius}m, Surface Force: {desired_force}N")
    ...
    logger.info(f"[Exp 1] Starting simulation loop for {sim_time}s ({n_steps} steps)...")
    ...
    logger.info("[Exp 1] Simulation completed successfully.")
    ...
    if save_plot:
        plot_experiment_1(logs, surface_box_cfg, circle_center, radius)
...
def plot_experiment_1(
    ...
    fig.suptitle("Experiment 1: Circular Surface Trajectory & Surface Force Task Diagnostics", fontsize=14, fontweight="bold")

    # Panel 1: XY Surface Circle Trajectory Path
    ax1 = axs[0, 0]
    ...
    ax1.set_title("1. Planar Trajectory Path (XY)")
    ax1.grid(True)
    ax1.legend()
    ax1.axis("equal")
    ...
    ax2.set_title("2. Level 0 Task: Z Position & Normal Force")
    ax2.grid(True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Experiment 1: Circular Surface Trajectory & Contact Force Control")
