...
def main():
    ...
    if args.inprocess:
        ...
        logger.info("\n>>> [1/11] Running Exp 1: Surface Circle & Force Exertion...")
        run_experiment_sub("exp1_surface_circle.py", ["--time", str(8.0 * scale), "--device", device, "--no-vis"])

        logger.info("\n>>> [2/11] Running Exp 2: Blocked Circle & Obstacle Compliance...")
        run_experiment_sub("exp2_blocked_circle.py", ["--time", str(8.0 * scale), "--device", device, "--no-vis"])

        logger.info("\n>>> [3/11] Running Exp 3: Reactive APF Obstacle Avoidance...")
        run_experiment_sub("exp3_apf_avoidance.py", ["--time", str(6.0 * scale), "--device", device, "--no-vis"])

        logger.info("\n>>> [4/11] Running Exp 4: Multi-Link Disturbance Rejection...")
        run_experiment_sub("exp4_multilink_push.py", ["--time", str(8.0 * scale), "--device", device, "--no-vis"])

        logger.info("\n>>> [5/11] Running Exp 5: Torque-Constrained Surface Wiping...")
        run_experiment_sub("exp5_torque_constrained_wipe.py", ["--time", str(8.0 * scale), "--device", device, "--no-vis"])

        logger.info("\n>>> [6/11] Running Exp 6: Kinematic Singularity Tracking...")
        run_experiment_sub("exp6_singularity_tracking.py", ["--time", str(6.0 * scale), "--device", device, "--no-vis"])
        ...
        logger.info("\n>>> [9/11] Running Exp 9: Energy Tank & Passivity Profiling...")
        run_experiment_sub("exp9_passivity_energy_profiling.py", ["--time", str(6.0 * scale), "--device", device, "--no-vis"])

        logger.info("\n>>> [10/12] Running Exp 10: Model Parameter Robustness...")
        run_experiment_sub("exp10_parameter_robustness.py", ["--time", str(6.0 * scale), "--device", device])

        logger.info("\n>>> [11/12] Running Exp 11: Prioritized Z-Corridor & Out-of-Bounds Circle...")
        run_experiment_sub("exp11_cartesian_corridor_circle.py", ["--time", str(8.0 * scale), "--device", device, "--no-vis"])
